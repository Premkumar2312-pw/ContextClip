...
class VisionRequest(BaseModel):
    prompt: str = Field(..., description="Prompt text to send with the image")
    image: str = Field(..., description="Image Data URL (e.g. data:image/png;base64,...) or image URL")
...
def vision(request: VisionRequest):
    ...
    if not request.image or not request.image.strip():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Image data cannot be empty or blank"
        )

    try:
        result = groq_service.generate_vision_response(request.prompt.strip(), request.image.strip())
        return GenerateResponse(response=result)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An unexpected error occurred while processing the AI vision request"
        )
